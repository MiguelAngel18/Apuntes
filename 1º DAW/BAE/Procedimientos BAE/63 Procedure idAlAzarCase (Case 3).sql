USE world;
DROP PROCEDURE IF EXISTS idAlAzarCase;
DELIMITER $$
CREATE PROCEDURE idAlAzarCase()
BEGIN
    ...
    CASE
		WHEN codigoCiudad>1 AND codigoCiudad<100 THEN 
			SELECT * FROM city WHERE id<=codigoCiudad;
    	WHEN codigoCiudad=REVERSE(codigoCiudad) AND codigoCiudad>9 THEN
			SELECT id, name FROM city WHERE id=REVERSE(id) AND codigoCiudad>9;
    	WHEN codigoCiudad=maximo OR codigoCiudad=1 THEN
